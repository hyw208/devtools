...
"""
i need to a python queue to add jobs to it and get a job out. 


1. This queue uses postgres sql db to make sure it doesn't lose any info upon restart. it should allow developer to specify the table name. this queue will be created multiple times across multiple python processes so i want to ensure db handlng is optimized
2. I want to use the connection pool and get connection out of the pool. If connection to db is lost, it should reconnect and restore connection pool. we should retry 3 times before aborting getting connection
3. the method to add job should be called 'add_job' 
4. the method to get a job should be called 'claim_next_job' where it get the oldest job so it is first come first picked up. The method should log claim datetime, and accept worker name. eligible jobs can be jobs never picked up or jobs processed by workers but not completed in time.
5. there should be a method to complete a job with status of processing, eg. success or failure with error message. Error message should be stored in its own column instead of merging into the job data. if a worker tries to complete a job beyond allowed time and the job has not been reclaimed by other worker, it's allowed to be completed by existing worker but we need to add message, eg. 'late to complete by worker xyz' to 'late' column. If the job has been reclaimed by other worker, we can only append to 'late' column wihtout updating status, done_at columns
...
def app():
    api = FastAPI()
    # from devtools.queue import PersistentQueue
    ...
    @api.post("/jobs")
    async def add_job(req: Req) -> dict: 
        # Add a new job
        """
    ...
    @api.get("/jobs/any")
    async def get_any_job(worker: str) -> dict: 
        """
    ...
    @api.put("/jobs/{job_id}")
    async def update_job(job_id: str, req: Req) -> dict:
        """
